...
\begin{figure}[!ht]
  ...
  \caption{Airflow during sleeping, \cite{}}
  \label{fig:airflow}
\end{figure}
...
First it is necessary to generate a sample wave. For the matter of simplicity the signal is generated using 2 sine waves as shown in \eqref{eq:sample_sine}. The result is displayed in figure \ref{fig:sines}. The lower plot shows the same signal as on the upper plot, plus some random normally distributed noise. Further we will use the noisy data to do preprocessing.
